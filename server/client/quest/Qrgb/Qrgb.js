...
s.newMap('blueSwitchCave',{
	...
	load:function(spot){
		...
		m.spawnActor(spot.ed,'death',{deathEvent:'killBlue'});
	...
	playerEnter:function(key){
		...
		s.addTorchEffect(key,'darker');
	},
	playerLeave:function(key){
		s.removeTorchEffect(key,'darker');
	}
});
...
s.newMap('greenSwitchForest',{
	...
	load:function(spot){
		...
		m.spawnActor(spot.ef,'bee',{tag:{monster:true},deathEvent:'killGreen'});
		...
		m.spawnActor(spot.ei,'bird',{tag:{monster:true},deathEvent:'killGreen'});
