...
eval(loadDependency(['ReputationGrid','Message','Actor','Main'],['ReputationConverter']));
if(SERVER) eval('var ReputationConverter; var Img = {drawIcon:{html:function(){}}};');	//BADD
...
ReputationConverter = exports.ReputationConverter = function(id,name,description,func,getButtonAppend){
	var tmp = {
		id:id || '',
		name:name || '',
...
ReputationConverter('dmgMeleeRange','Dmg ML -> Dmg RG','Convert every Dmg Melee Boost into Dmg Range Boost',
	helper('dmg-melee-+','dmg-range-+'),
	function(){ return helper2(Stat.get('dmg-melee-+').icon,Stat.get('dmg-range-+').icon); }
);
ReputationConverter('dmgRangeMagic','Dmg RG -> Dmg MG','Convert every Dmg Range Boost into Dmg Magic Boost',
	helper('dmg-range-+','dmg-magic-+'),
	function(){ return helper2(Stat.get('dmg-range-+').icon,Stat.get('dmg-magic-+').icon); }
);
ReputationConverter('dmgMagicMelee','Dmg MG -> Dmg ML','Convert every Dmg Magic Boost into Dmg Melee Boost',
	helper('dmg-magic-+','dmg-melee-+'),
	function(){ return helper2(Stat.get('dmg-magic-+').icon,Stat.get('dmg-melee-+').icon); }
);

ReputationConverter('dmgFireCold','Dmg FR -> Dmg CD','Convert every Dmg Fire Boost into Dmg Cold Boost',
	helper('dmg-fire-+','dmg-cold-+'),
	function(){ return helper2(Stat.get('dmg-fire-+').icon,Stat.get('dmg-cold-+').icon); }
);
ReputationConverter('dmgColdLightning','Dmg CD -> Dmg LG','Convert every Dmg Cold Boost into Dmg Lightning Boost',
	helper('dmg-cold-+','dmg-lightning-+'),
	function(){ return helper2(Stat.get('dmg-cold-+').icon,Stat.get('dmg-lightning-+').icon); }
);
ReputationConverter('dmgFireLightning','Dmg LG -> Dmg FR','Convert every Dmg Lightning Boost into Dmg Fire Boost',
	helper('dmg-lightning-+','dmg-fire-+'),
	function(){ return helper2(Stat.get('dmg-lightning-+').icon,Stat.get('dmg-fire-+').icon); }
);

ReputationConverter('hpLeech','Hp -> Life Leech','Convert every Hp Boost into Life Leech Boost',
	helper(['hp-max','hp-regen'],['leech-chance','leech-magn']),
	function(){ return helper2(Stat.get('hp-max').icon,Stat.get('leech-magn').icon);}
);
ReputationConverter('leechMana','Life Leech -> Mana','Convert every Life Leech Boost into Mana Boost',
...
ReputationConverter('manaHp','Mana -> Hp','Convert every Mana Boost into Hp Boost',
	helper(['mana-max','mana-regen'],['hp-max','hp-regen']),
	function(){ return helper2(Stat.get('mana-max').icon,Stat.get('hp-max').icon);}
);

ReputationConverter('critAmount','Crit -> Bullet Amount','Convert every Crit Boost into Bullet Amount Boost',
...
ReputationConverter('amountSpd','Bullet Amount -> Atk Spd','Convert every Bullet Amount Boost into Atk Spd Boost',
	helper(['bullet-amount'],['atkSpd']),
	function(){ return helper2(Stat.get('bullet-amount').icon,Stat.get('atkSpd').icon);}
);
ReputationConverter('spdCrit','Atk Spd -> Crit','Convert every Atk Spd Boost into Crit Boost',
