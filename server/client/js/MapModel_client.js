...
MapModel = function(quest,name,x,y,grid){
	var tmp = {
		id:quest + '-' + name,
		graphicPath:'',
		quest:quest,
...
MapModel.useSignInPack = function(nameList){
	for(var i in nameList){
		if(i === nameList[i].graphic){	//base map
			...
			var grid = MapModel.Grid.uncompress(nameList[i].gridPlayer,widthSq,heightSq);
			
			MapModel(i.split('-')[0],i.split('-')[1],width,height,grid);
		}
	}
...
MapModel.Grid.uncompress = function(compressed,width,height){ //compress in MapModel
	var oneD = Tk.ABC.toBinary(compressed);
